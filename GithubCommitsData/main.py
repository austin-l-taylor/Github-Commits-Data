...
# Configuration
SECRET_ID = "8rOo-f_zMmozAZaToI6XnQ"

Keeper.authorize()
secret = Keeper.client.get_secrets([SECRET_ID])[0]

access_token = secret.field("keyPair", single=True)["privateKey"]
server = secret.custom_field("Server", field_type=None, value=None)
database = secret.custom_field("Database", field_type=None, value=None)
username = secret.field("login", single=True)
password = secret.field("password", single=True)

str_server = server[0]
str_database = database[0]
...
def sql_push(df):
    ...
    try:
        ...
        for _, row in df.iterrows():
            cursor.execute(
                """
                INSERT INTO GitHub.CommitHistory (Organization, Project, commitSHA, commitDate, commitName, commitEmail, commitMessage, commitURL, numAdditions, numDeletions, totalChanges, branch) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                row["Organization"],
                row["Project"],
                row["commitSHA"],
...
def sql_stored_procedure():
    ...
    try:
        ...
        cursor.execute("EXEC MWA.GitHub.GetProjectAndMostRecentCommit")
...
def main():
    ...
    repo_list = fetch_repos(ORG_NAME) # Fetch the list of repositories for the organization
    repo_data = sql_stored_procedure() # Retrive commit data
    
    project_names = [repo[1] for repo in repo_data] # Extracts Repo Names from Stored Proc
    ...
    branch_dict = {} # Dictionary to store branches for each repo
